...
void gen_number(uv_timer_t *timer) {
	...
	if ( wiringPiSetup() == -1 )
                printf("DHT11 sensor is not working");
         
         read_dht_data(&temp,&hum);
         printf( "Humidity = %.1f %% Temperature = %.1f *C \n", hum,temp);
     //    printf("Hi ravi, Temp is : %.1f *c\n",temp);
	//	 printf("Hi ravi, Humidity is : %.1f %% \n",hum);
         delay( 2000 ); /* wait 2 seconds before next read */
	
	//float x=temp;        

    //double x = rand() / 1000000.0;
    dslink_node_update_value_new(link, node, json_real(temp));

 /*   if (x > 1000.0) {
        dslink_node_set_meta_new(link, node, "@number", json_real(temp));
    } else {
...
void responder_init_rng(DSLink *link, DSNode *root) {
    DSNode *num = dslink_node_create(root, "Temperature", "node");
    if (!num) {
